...
function runGenericSensorTests(sensorData, readingData) {
  ...
  // This test does not make much sense, as it does not match any error
  // condition described in the spec. Passing `connected: false` to
  // create_virtual_sensor() is enough to make start() fail. sensor_test(async
  // (t, sensorProvider) => {
  ...
  // }, `${sensorName}: Test that onerror is sent when sensor is not
  // supported.`);
